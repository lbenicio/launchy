...
final class LaunchyViewModel: ObservableObject {
    ...
    /// Spotlight search results when search is active
    @Published var spotlightResults: [SpotlightResult] = []
    @Published var isSearchingSpotlight: Bool = false
    ...
    func pagedItems(matching query: String) -> [[LaunchyItem]] {
        ...
        for item in items {
            switch item {
            case .app(let icon):
                if let score = icon.name.fuzzyMatch(normalized), score > 0.3 {
        ...
        let capacity = settings.pageCapacity
        guard capacity > 0 else { return [combinedResults] }
        let chunks = combinedResults.chunked(into: capacity)
    ...
    init(
        ...
        let coordinator = DragCoordinator(viewModel: self)
