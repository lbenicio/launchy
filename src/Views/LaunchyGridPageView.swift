...
struct LaunchyGridPageView: View {
    ...
    private func adjustedPadding(for metrics: GridLayoutMetrics) -> CGFloat {
        let basePadding = metrics.padding
        let topBarHeight: CGFloat = 60  // Height of wiggle mode message bar
        let bottomBarHeight: CGFloat = 80  // Height of wiggle mode delete bar

        let topAdjustment = viewModel.isEditing ? topBarHeight : 0
        let bottomAdjustment = viewModel.isEditing ? bottomBarHeight : 0

        return max(basePadding, max(topAdjustment, bottomAdjustment))
    }
    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            .padding(.horizontal, adjustedPadding(for: metrics))
            .padding(.vertical, adjustedPadding(for: metrics))
    ...
    private func launchyTile(for item: LaunchyItem) -> some View {
        ...
        let baseContent = LaunchyItemView(
            ...
            onMoveLeft: { id in
                withAnimation(Self.rearrangeSpring) { viewModel.shiftItem(id, by: -1) }
            },
            onMoveRight: { id in
                withAnimation(Self.rearrangeSpring) { viewModel.shiftItem(id, by: 1) }
            },
