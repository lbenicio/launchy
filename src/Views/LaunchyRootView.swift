...
struct LaunchyRootView: View {
    ...
    var body: some View {
        ...
        .frame(minWidth: 1024, minHeight: 720)
            ...
            .background(
                WindowConfigurator(
                    preferredWindowSize: settingsStore.settings.windowedSize,
        ...
        .onReceive(AppCoordinator.shared.events) { event in
            ...
            case .navigatePrevious:
                viewModel.goToPreviousPage(totalPages: max(pages.count, 1))
            case .navigateNext:
                viewModel.goToNextPage(totalPages: max(pages.count, 1))
            default:
                break
            }
        }
    ...
    #if os(macOS)
        @ViewBuilder
        private func themedBackgroundContent() -> some View {
        ...
        private func folderNotchAnchorX(edgePadding: CGFloat) -> CGFloat {
            guard let screenX = viewModel.folderOpenScreenX,
                let window = NSApp.windows.first(where: {
                    $0.identifier?.rawValue == "com.launchy.app.main"
                })
            else { return 0.5 }
    ...
    private func dismissLauncher() {
        // For a normal app, just quit when user wants to close
        #if os(macOS)
            NSApp.terminate(nil)
        #endif
    }
