...
final class AppCatalogStore: ObservableObject {
    ...
    private var folderDragContext: FolderDragContext?
    private var lastReorderPair: (draggingID: String, targetID: String)?
    ...
    func beginDraggingAppFromFolder(folderID: String, appID: String) {
        ...
        guard
            ...
            case let .folder(originalFolder) = rootEntries[folderIndex],
            let appIndex = originalFolder.apps.firstIndex(where: { $0.id == appID }) else { return }
    ...
    private func moveEntry(
        ...
        guard
            ...
            let targetIndex = rootEntries.firstIndex(where: { $0.id == targetID }) else { return false }

        let destination: Int = if fromIndex < targetIndex {
            targetIndex - 1
        } else {
            targetIndex
        }
    ...
    private func mergeEntry(draggingID: String, targetID: String) -> Bool {
        ...
        case (.app(let app), var .folder(folder)):
            withAnimation(.easeInOut(duration: 0.18)) {
            ...
            persistLayout()
            return true
        case let (.app(sourceApp), .app(targetApp)):
            ...
            persistLayout()
            return true
        case (.folder(let sourceFolder), var .folder(targetFolder)):
            withAnimation(.easeInOut(duration: 0.18)) {
    ...
    private func applySavedLayout(to entries: [CatalogEntry]) async -> [CatalogEntry] {
        ...
        let snapshotFolderIDs = Set(snapshot.entries.compactMap(\.folderID))
    ...
    func dissolveFolder(id folderID: String) {
        ...
        guard
            ...
            case let .folder(folder) = rootEntries[index] else { return }
